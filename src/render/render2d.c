...
/*   Updated: 2023/11/10 21:30:54 by isromero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_horizontal_intersection(t_map *map, int player_x, int player_y, double angle)
{
	printf("Angle: %f\n", angle);
    printf("tan(angle): %f\n", tan(angle));
    printf("Player_x: %d, Player_y: %d\n", player_x, player_y);
	if (angle >= 0 && angle < M_PI) // Cuando un rayo mira hacia arriba se verifica en términos geométricos: [0, π)
		map->line->intersection_y = ((player_y / 16) * (16) - 1) / 16;
	else // el rayo mira hacia abajo
		map->line->intersection_y = ((player_y / 16) * (16) + 16) / 16;
	map->line->intersection_x = (player_x + (player_y - map->line->intersection_y) / tan(angle)) / 16;
}
...
void	render_ray_2d(t_map *map, int player_x, int player_y, double angle)
{
	...
	{
		if (map->map[(int)ray_y / 16][(int)ray_x / 16] == '1') // Implementación simple para parar el rayo cuando encuentra un muro(se necesita hacer algo más avanzado)
			break;
		find_horizontal_intersection(map, player_x, player_y, angle);
		printf("(%d ", map->line->intersection_x);
		printf("%d)\n", map->line->intersection_y);
